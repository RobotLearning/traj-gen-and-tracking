function [err,u_app] = oldILC(t,x_nom,u_trj,fun,STR,num_iters)
...
u_app = zeros(dim_u, Nu, num_iters); 
% store errors (traj. deviation) here
err = Inf(dim_x*Nu,num_iters); 
...
while iter <= num_iters
    ...
    u = quadprog(2*(F'*S')*S*F + 2*a2*(D2'*D2), 2*F'*S'*d, [], [], [], [], [], [], [], options);
