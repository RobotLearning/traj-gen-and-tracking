...
function [qf,qfdot,T,Tland] = lazyPlayer(robot,models,q0)
...
table_z = models.floor.level + models.table.height;
...
function [c,c_eq,Gc,Gc_eq] = strikeConstraints(robot,models,Q1,T,Q0)
...
a3ret = (2/(Tret^3))*(q1-q0) + (1/(Tret^2))*(q1dot + q0dot);
a2ret = (3/(Tret^2))*(q0-q1) - (1/Tret)*(q0dot + 2*q1dot);
% compute the extrema
t1ret = (-a2ret + sqrt(a2ret.^2 - 3*a3ret.*q1dot))./(3*a3ret);
t2ret = (-a2ret - sqrt(a2ret.^2 - 3*a3ret.*q1dot))./(3*a3ret);
...
qext_ret = @(t) a3ret.*(t.^3) + a2ret.*(t.^2) + q1dot.*t + q1;
con = robot.CON;
...
task_ineq = [distBall2RacketAlongNormal - ball_radius;
             vecBallAlongRacket(1:2)'*vecBallAlongRacket(1:2) - racket_radius^2;
             -vecBallAlongRacket(1:2)'*vecBallAlongRacket(1:2);
             -time2Net;
             xNet - table_xmax;
             -xNet - table_xmax;
             zNet - wall_z;
