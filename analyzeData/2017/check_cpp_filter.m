...
balls1 = remove_outliers(balls1{1});
balls3 = balls3{1};
t1 = balls1(:,1);
b1 = balls1(:,2:4);
t3 = balls3(:,1);
...
% GET CPP FILTERED BALL VALUES
file_filt = ['~/Dropbox/data/real_ball_data_0317/balls_', ...
        int2str(trial), '_filtered.txt'];  
...
ball_est_mat_net = filterBallsEKF(t_pre_net,balls3_pre_net,ekf,spin_state);
ball_pred_net = predictTillLastBlob(ekf,t_pred,t_pred(1),...
                             ball_est_mat_net(end,2:end));
% plot predicted balls
ball_preds_net = [ball_est_mat_net(:,2:end);ball_pred_net'];
subplot(2,2,1);
...
ball_est_mat_bounce = filterBallsEKF(t_pre_bounce,balls3_pre_bounce,ekf,spin_state);
ball_pred_bounce = predictTillLastBlob(ekf,t_pred,t_pred(1),...
                                       ball_est_mat_bounce(end,2:end));
ball_preds_bounce = [ball_est_mat_bounce(:,2:end);ball_pred_bounce'];
subplot(2,2,2)
...
b_filt_pre_bounce = b_filt(1:idx_bounce-1,:);
