% This file contains a sample implementation of a 'Vanilla' Policy
...
for iter=1:n_iter
    ...
    for batch=1:n_batch
        ...
        phi(n_rfs+1,batch)=sum(explore(batch,:).^2-std_dev^2)/std_dev^3;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    % compute the baseline
    for i=1:n_rfs+1
        baseline(i)=sum(phi(i,:).^2.*batch_return)/sum(phi(i,:).^2);
    end
    gradient=sum(phi.*(batch_return(ones(n_rfs+1,1),:)-baseline(:,ones(1,batch))),2)./n_batch;
