...
Solution of \eqref{costFnc1} using Pontryagin's maximum principle is well known in the optimal control literature: the optimal $\joint(t)$ is a third degree polynomial for each degree of freedom, with the equality constraints \eqref{transCond1} - \eqref{transCond2} imposing a \emph{transversality conditions} on the Hamiltonian to satisfy at striking time~\cite{Schaettler12}. See Figure~\ref{mainIdea} for an illustration. % which figure 
...
During test time we use the same trained Extended Kalman Filter (EKF) to perform prediction on the filtered ball state. 
