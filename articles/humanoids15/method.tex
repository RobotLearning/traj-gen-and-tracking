...
\begin{algorithm}[tb]
   ...
   \label{alg1}
\begin{algorithmic}
   \STATE {\bfseries Input:} $\threshold > 0$, $\vec{Q}, \vec{R} \succeq 0$, $\dmp$ 
   ...
   \REPEAT 
 	   ...
 	   \STATE Apply controls $\sysInput = \trjInput + \ilcInput$ to $\ddot{\joint} = \dynamics(\joint,\dot{\joint},\sysInput)$ %\sysInput = \nu - K_{\sysInput}(\state - \dmp(\weights)))$
