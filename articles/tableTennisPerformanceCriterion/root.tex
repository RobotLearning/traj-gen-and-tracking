...
% Inputs:
%
% 1. Estimates of ball position & velocity till net N
% 2. Stability region of the table S_T is fixed as 3d-coordinates
% 3. Flight Model of ball dynamics bdot = f(b) estimated from ball trajectory data
% 4. Bounce model of ball dynamics bdot = g(b) estimated from ball trajectory data
% 5. Extended Kalman filter or a regression method to predict ball trajectories
%
% Estimates
% 
% 1. A set or a probability distribution of incoming ball states parameterized by t, e.g. b_t \approx N(\mu_b_in(t),\sigma_b_in(t)) 
% 2. Interaction probabilities (ball touching the racket)
% 3. A probabilistic model (e.g. a Gaussian Process) describing the interaction model: outputs a set or a probability distribution of outgoing ball states parameterized (or indexed) by t, e.g. b_t \approx N(\mu_b_out(t), \sigma_b_out(t))
...
% Reasons for uncertainty & probabilistic modeling:
% 1. Ball cannot be estimated precisely [both flight/bounce models are also inexact]
% 2. Trajectories cannot be tracked [control errors]
% 3. Racket-ball interaction model not known well [elastic momentum law, velocity dependent coefficient of restitution]
%
...
% coefficient of restitution in the racket: o_parallel - v = \epsilon_{R} (-i_parallel + v) with v the speed of the racket along its normal
