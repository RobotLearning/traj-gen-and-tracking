%% Test prediction error reduction with ball correction on data
...
ball_lookup_SL = ballEst(idx_filter_SL,2:end);
...
%% Try to get the same with the polyfilter class

dim = 3; % 2D
order = 2; % 2nd order polynomial
size_mov_avg = 12; % size of the moving average

filter_poly = PolyFilter(dim,order,size_mov_avg,funState);
% find the index of the lookup ball estimate
idx_filter_matlab = find(b3plot(:,2) > y_center,1);
idx_filter_matlab_start = idx_filter_matlab - size_mov_avg + 1;
dtPolyFilter = 0.0;
for i = 1:size_mov_avg
    filter_poly.update(dtPolyFilter,b3plot(idx_filter_matlab_start+i-1,:));
    dtPolyFilter = t3plot(idx_filter_matlab_start+i) - t3plot(idx_filter_matlab_start+i-1);
end
ball_lookup_matlab = filter_poly.x;
...
[~,idx_bounce] = min(b3plot(:,3));
dtPredTillBounce =  t3plot(idx_bounce) - t_lookup;
dtPredTillLastBlob3 = t3plot(end) - t_lookup;
dtPredTillLastBlob1 = t1plot(end) - t_lookup;

%% Predict using ball estimate
dt = 1/60;
initVar = 1;
filter.initState(ball_lookup_matlab(:),initVar);
...
for i = 1:N_updates
    ball_lookup_matlab = filter_poly.x;
    filter.initState(ball_lookup_matlab(:),0.01);
    filter.linearize(dt,0);
    [ballPred,ballTime,numBounce,time2PassTable] = ...
                predictBallPath(dt,predictHorizon,filter,table);
    N = size(b3plot,1) - (idx_filter_matlab + i);
    x_pred = ballPred(1:3,1:N)';
    x_meas = b3plot(idx_filter_matlab+i:idx_filter_matlab+i+N-1,:);
