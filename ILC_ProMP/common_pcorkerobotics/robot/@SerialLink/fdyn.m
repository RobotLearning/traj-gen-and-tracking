...
function [t, q, qd] = fdyn(robot, tref, torqfun, q0, qd0, varargin)
    ...
    if isfield( varargin{:}, 'ode' )
        options = odeset('RelTol',varargin{:}.ode.rel_tol, ...
                         'AbsTol',varargin{:}.ode.abs_tol);
    else
        options = odeset('RelTol',1e-2, 'AbsTol', 1e-2);
    end
    
    [t,y] = ode23(@fdyn2, tref, q0, options, robot, torqfun, varargin{:});
    %[t,y] = ode45(@fdyn2, tref, q0, options, robot, torqfun, varargin{:});
    q  = y(:,1:n);
    qd = y(:,n+1:2*n);
